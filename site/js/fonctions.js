...
const baseUrl = "https://asj-corsproxy.herokuapp.com/https://api.deezer.com/";
const loader = document.getElementById("loader");
const sideSearchInput = document.getElementById("side-search");
let urlParam = "";
const player = document.getElementById("play-song");
const song = document.getElementById("audio-song");
const range = document.getElementById("song-timeline");
const label = document.getElementById("label-range");
const labelDuration = document.getElementById("duration");
...
function loadTrack(track) {
  ...
  document.getElementById(
    "cover-container"
  ).innerHTML = `<img src="${track.album.cover_small}" alt="${track.title}"/>`;
...
function playSong(e) {
  fetch(`${baseUrl}track/${e.id}`)
    ...
    .catch((err) => {
      errorFetch("info-container__title");
    });
}
...
function errorPage(containerId) {
  document.getElementById(containerId).innerHTML =
    "<h1 class='error-title'>Désolé, nous ne trouvons pas la page que vous recherchez</h1>";
}
/**
 * @function errorFetch
 * @description displays a error message
 * @param {string} containerId
 */
function errorFetch(containerId) {
  document.getElementById(containerId).innerHTML =
 ...
 */
document.getElementById("side-submit").addEventListener("click", () => {
  if (sideSearchInput.value) {
    urlParam += `?q=${sideSearchInput.value}`;
    location.href = `recherche.html${urlParam}`;
  }
