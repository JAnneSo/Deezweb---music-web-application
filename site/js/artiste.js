    ...
    .then((artist) => {
      ...
      if (!artist.error) {
        ...
        document.querySelector(
          ".blur"
        ).style.background = `center / cover no-repeat url(${artist.picture_big})`;

        // API call to get the artist's last album
        fetch(`${baseUrl}artist/${id}/albums`)
          .then((response) => response.json())
          .then((albums) => {
            let lastAlbumTitleElt = document.createElement("h2");
            lastAlbumTitleElt.textContent = "Dernier album";
            document
              .getElementById("popular-album")
              .appendChild(lastAlbumTitleElt);
            document
              .getElementById("popular-album")
              .appendChild(createAlbumCard(albums.data[0]));
            // API call to get the artist's top tracks
            fetch(`${baseUrl}artist/${id}/top`)
              .then((response) => response.json())
              .then((topTracks) => {
                ...
                document
                  ...
                  .appendChild(popularTracksTitleElt);
                for (let i = 0; i < topTracks.data.length; i++) {
                  document
                    .getElementById("popular-tracks")
                    .appendChild(createListCard(i + 1, topTracks.data[i]));
                }
              })
              .catch((err) => {
                console.log(err);
