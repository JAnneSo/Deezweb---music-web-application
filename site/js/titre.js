    ...
    .then((track) => {
      ...
      if (!track.error) {
        ...
        trackInfoCtnr.innerHTML = `
            ${
              track.explicit_lyrics
                ? "<p class='explicit-tag'>Explicit</p>"
                : ""
            }
                    <h1 class="track__info--title">${track.title}</h1>
                    <h2 class="track__info--album"><a href="album.html?id=${
                      track.album.id
                    }">${track.album.title}</a></h2>
                    <p class="track__info--date"><span>${new Date(
                      track.release_date
                    ).getFullYear()}</span><span>${formatSeconds(
          track.duration
        )}</span></p>
                    <p class="btn-ctnr">
                        <button id="${
                          track.id
                        }" class="red-btn" onclick="playSong(this)"><i class="fas fa-play"></i>Écouter</button>
                        <a href="${
                          track.link
                        }" target="blank" class="red-btn"><i class="fab fa-deezer"></i>Voir le titre sur Deezer</a>
                    <input type="checkbox" id="ckbx_${
                      track.id
                    }" onclick="storeFavorite(this)" />
                    <label for="ckbx_${
                      track.id
                    }" class="track-card__love"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">' +
        ...
        document.getElementById(
          "card-container"
        ).innerHTML = `<div class="popular-card">
